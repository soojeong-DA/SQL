...
-- 1. 정확률(Precision)을 사용해 검색의 타당성 평가
-- -- 정확률(정밀도): 검색 결과에 포함되는 아이템 중 정답 아이템이 어느 정도 비율로 포함되는지
-- -- 앞서 구한 재현율과는 비슷하나, 분모가 다름(n순위까지의 누계 item 수)
WITH
search_result_with_correct_items AS (
	SELECT COALESCE(r.keyword, c.keyword) AS keyword,
		r.rank,
...
-- 2. 정확률 값을 집약해서 비교하기 쉽게 만들기 
-- 2-1. 검색 결과 상위 5개의 정확률을 키워드별로 추출
WITH
search_result_with_correct_items AS (
	SELECT COALESCE(r.keyword, c.keyword) AS keyword,
		r.rank,
...
, precision_over_rank_5 AS (
	SELECT keyword,
		rank,
		precision,
		-- 검색 결과 순위가 높은 순서로 번호 붙이기 (결과에 나오지 않는 것은 0으로)
		ROW_NUMBER() OVER(PARTITION BY keyword ORDER BY COALESCE(rank, 0) DESC)
		AS desc_number
	FROM search_result_with_precision
	WHERE COALESCE(rank,0) <= 5  -- 검색 결과 상위 5개 이하 or 검색 결과에 포함되지 않는 item만
)
SELECT keyword,
